...
/**
 * TamperAlarm renders a full-screen alert state with a breathing red border
 * and a glass-darkened backdrop if a checksum verification failure occurs.
 */
export function TamperAlarm() {
  ...
  return (
    <AnimatePresence>
      {activeViolationIds.length > 0 && (
        <>
          {/* 1. Immersive screen-wide breathing Red Security Perimeter */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ 
              opacity: [0.4, 0.8, 0.4],
              boxShadow: [
                ...
                'inset 0 0 60px rgba(239, 68, 68, 0.55)',
                'inset 0 0 30px rgba(239, 68, 68, 0.25)'
              ]
            }}
            exit={{ opacity: 0 }}
            transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
            className="fixed inset-0 z-[9990] pointer-events-none border-4 border-red-500/40"
          />
          ...
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[9991] flex items-center justify-center p-6 bg-black/75 backdrop-blur-lg"
          >
            <motion.div 
            ...
            >
              ...
              <div className="flex items-start gap-4 mb-6">
                <motion.div 
                  ...
                  className="shrink-0 rounded-2xl bg-red-500/15 border border-red-500/30 p-3 shadow-[0_0_15px_rgba(239,68,68,0.2)] text-red-500"
                >
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-8 border-t border-white/5 pt-6">
                ...
                <Button
                  variant="danger"
                  onClick={lockVault}
                  className="flex-1 gap-2 py-3.5 text-xs font-bold uppercase tracking-wider"
                >
                  <Lock size={14} />
                ...
                <Button
                  ...
                  className="flex-1 gap-2 py-3.5 border-white/10 hover:border-red-500/20 text-white/60 hover:text-white text-xs font-bold uppercase tracking-wider"
                >
                  <XCircle size={14} />
                  Acknowledge & Inspect
                </Button>
